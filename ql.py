...
def greedy(state):
    if(max(Q[state]>0)):
        return actions_list2[np.argmax(Q[state])]
...
for i in xrange(100):
    ...
    #random_movements.append(num_mov)
    greedy_movements.append(num_mov)
    #eGreedy_movements.append(num_mov)
    num_mov = 0
print Q
...
#graph_random, = plt.plot(random_movements)
graph_greedy, = plt.plot(greedy_movements)
#graph_eGreedy, = plt.plot(eGreedy_movements)

plt.legend(handles=[graph_greedy],
labels=["Greedy"])
